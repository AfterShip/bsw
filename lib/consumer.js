...
class Consumer extends AbstractClient {
	...
	// callback function called before a job is reserved
	async _onBeforeJobReserved() {
		this.log('before job reserved');
	}

	async _onAfterJobReserved(job) {
		this.log('after job reserved, job_id=', job.job_id);
	}
